...
# chat-spin-api-user-match-handler fn
resource "aws_lambda_function" "chat_spin_api_user_match_handler" {
  function_name    = "${var.prefix}-chat-spin-api-user-match-handler"
  s3_bucket        = aws_s3_bucket.chat_spin_api_lambda_bucket.id
  s3_key           = aws_s3_object.chat_spin_api_user_match_handler.key
  handler          = "index.handler"
  runtime          = "nodejs20.x"
  role             = aws_iam_role.chat_spin_api_user_match_handler_role.arn
  timeout          = 5
  source_code_hash = data.archive_file.chat_spin_api_user_match_handler.output_base64sha256
...
resource "aws_cloudwatch_log_group" "chat_spin_api_user_match_handler_log_group" {
  name              = "/aws/lambda/${aws_lambda_function.chat_spin_api_user_match_handler.function_name}"
  retention_in_days = 30
}

resource "aws_lambda_permission" "allow_apigateway_user_match" {
  statement_id  = "AllowAPIGatewayInvokeUserMatch"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.chat_spin_api_user_match_handler.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_apigatewayv2_api.chat_spin_api.execution_arn}/*/*"
}